...
void update_gdi_cache_bitmap_v3(rdpContext* context, CACHE_BITMAP_V3_ORDER* cache_bitmap_v3)
{
	...
	/*
	 * XXX A RFX mode hack
	 *     Turn on RFX codec only if server encode it.
	 */
	if (bitmapData->codecID != 0) {
		Bitmap_SetDimensions(context, bitmap, 64, 64);
		cache->settings->rfx_codec_mode |= 0x100;
	}

	bitmap->Decompress(context, bitmap,
			bitmapData->data, bitmap->width, bitmap->height,
			bitmapData->bpp, bitmapData->length, 1);

	bitmap->New(context, bitmap);

	if (bitmapData->codecID != 0) {
		Bitmap_SetDimensions(context, bitmap, bitmapData->width,
				     bitmapData->height);
		cache->settings->rfx_codec_mode &= 0xff;
	}
